...
export function Lightbox({ items, index, onClose, onChange }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-onyx/95 backdrop-blur-md flex items-center justify-center p-4">
      <button onClick={onClose} aria-label="Cerrar" className="absolute top-4 right-4 text-ivory hover:text-gold p-2">
        <X className="h-6 w-6" />
      </button>
